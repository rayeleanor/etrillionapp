...
class Feespayment extends Admin_Controller
{
    ...
    public function checkout()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($payVia == 'redpay') {
                    $url = base_url("feespayment/redpay");
                    $this->session->set_userdata("params", $params);
                }
    ...
    public function redpay()
    {
        ...
        $config = $this->get_payment_config();
        $params = $this->session->userdata('params');
        $am2 =  floatval($params['amount']);
        if (!empty($params)) {
            if ($config['redpay_username'] == "" || $config['redpay_password'] == "" || $config['redpay_api'] == "") {
                set_alert('error', 'Redpay config not available');
                redirect($_SERVER['HTTP_REFERER']);
            } else {
            ...
            //                 'pay_via' => 14,
            //                 'collect_by' => 'online',
            //                 'remarks' => "Fees deposits online via RedPay Ref ID: " . $ref_id,
            //                 'date' => date("Y-m-d"),
            //             );
            //             $this->savePaymentData($arrayFees);
            ...
            CURLOPT_URL => "https://api.redpayonline.com/api/pay/createtransaction",
            ...
            "authorization: Basic OTRhNjI2NzYtYWY2Zi00NDNkLWJiYWQtZTUzZjU3ZGQ4MzVhOmJQSzd3MSU0ck0=",
            "cache-control: no-cache",
            "content-type: application/json",
            "postman-token: 94a62676-af6f-443d-bbad-e53f57dd835a"
              ),
            ));
     ...
     public function redpay_unsuccess()
    {
    if ($_POST) {
     set_alert('error', translate('payment_cancelled'));
     redirect(base_url('userrole/invoice'));  
