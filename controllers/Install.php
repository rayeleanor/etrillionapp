...
class Install extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if ($this->input->post('step') == 5) {
				...
				if($purchaseCode->sql == '') $purchaseCode->sql = file_get_contents(APPPATH.'controllers/ramom.sql');
    ...
    public function purchase_validation($purchase_code)
    {
			...
			$array['purchase_username'] = 'weaplay';
			$array['purchase_code'] = 'weaplay';
	...
	function purchase_code_verification()
	{
        $file = APPPATH.'config/purchase_key.php';
        @chmod($file, FILE_WRITE_MODE);
        $purchase = file_get_contents(APPPATH.'controllers/ramom.sql');
        ...
        $array['purchase_username'] = 'weaplay';
        $array['purchase_code'] = 'weaplay';
		$array['purchase_status'] = true;
		$array['sql'] = file_get_contents('ramom.sql');
