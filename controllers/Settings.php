...
class Settings extends Admin_Controller
{
    ...
    public function payment_active()
    {
        ...
        $redpay_status = isset($_POST['redpay_status']) ? 1 : 2;
        $arrayData = array(
    ...
    public function jazzcash_save()
    {
        ...
        if ($this->form_validation->run() !== false) {
            $sandbox = isset($_POST['jazzcash_merchant_id']) ? 1 : 2;
            $arraySSLcommerz = array(
     ...
     public function redpay()
    {
        ...
        $branchID = $this->application_model->get_branch_id();
        $this->form_validation->set_rules('redpay_username', 'Redpay UserName', 'trim|required');
        $this->form_validation->set_rules('redpay_password', 'Redpay Password', 'trim|required');
        $this->form_validation->set_rules('redpay_api', 'Redpay APi', 'trim|required');
        if ($this->form_validation->run() !== false) {
            $arraySSLcommerz = array(
                'redpay_username' => $this->input->post('redpay_username'),
                'redpay_password' => $this->input->post('redpay_password'),
                'redpay_api' => $this->input->post('redpay_api'),
            );
            $this->db->where('branch_id', $branchID);
            $q = $this->db->get('payment_config');
            if ($q->num_rows() == 0) {
